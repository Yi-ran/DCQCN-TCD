...
namespace ns3 {
	...
	public:
		...
		//NP-ECN
		uint32_t m_usedEgressPGPackets[pCnt][qCnt];
		uint32_t m_usedEgressPGPacketsCounter[pCnt][qCnt];
		uint32_t GetEgressPackets(uint32_t port, uint32_t qIndex);
	...
	//private
		...
		// used for calculate input queue grow rate
		uint32_t m_inputEgressBytes[pCnt][qCnt];
		double last_caltime[pCnt][qCnt];
		uint32_t last_inputEgressBytes[pCnt][qCnt];
