...
namespace ns3 {
	...
	BroadcomNode::BroadcomNode()
	{
		...
		//ingress params
		m_buffer_cell_limit_sp = 4 * 4000 * 1030; //ingress sp buffer threshold
		//m_buffer_cell_limit_sp_shared = 4000 * 1030; //ingress sp buffer shared threshold, nonshare -> share
		m_pg_min_cell = 2 * 1030; //ingress pg guarantee
		//m_pg_min_cell = 5 * 1030; //ingress pg guarantee
		m_port_min_cell = 2 * 1030; //ingress port guarantee
		//m_port_min_cell = 5 * 1030; //ingress port guarantee
		m_pg_shared_limit_cell = 2 * 160 * 1030; //max buffer for an ingress pg
		m_port_max_shared_cell =  2 * 4800 * 1030; //max buffer for an ingress port
		//m_port_max_shared_cell = 5 * 4800 * 1030; //max buffer for an ingress port
		//m_pg_hdrm_limit = 5 * 100 * 1030; //ingress pg headroom
		m_pg_hdrm_limit = 100 * 1030; //ingress pg headroom
		//m_port_max_pkt_size = 100 * 1030; //ingress global headroom
		//m_port_max_pkt_size = 5 * 100 * 1030; //ingress global headroom
		//still needs reset limits..
		//m_port_min_cell_off = 4700 * 1030;
		m_port_min_cell_off = 2 * 4700 * 1030;
	...
	//add by yiran: check  whether input rate is larger than line_rate. periodically check
	//wrong. we should check the grow rate of queue
	bool
		BroadcomNode::CheckEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize)
	{
		if(last_caltime[port][qIndex] == 0)
		{
			last_caltime[port][qIndex] = Simulator::Now().GetMicroSeconds();
			last_inputEgressBytes[port][qIndex] = m_usedEgressQSharedBytes[port][qIndex];
		}
		//m_inputEgressBytes[port][qIndex] += psize;
		double interval = Simulator::Now().GetMicroSeconds() - last_caltime[port][qIndex];
		if( interval >= 60)
		{
			//double inputrate = m_inputEgressBytes[port][qIndex] * 8.0 / 1000.0 / interval;
			double grow = 0;
			if(m_usedEgressQSharedBytes[port][qIndex] >= last_inputEgressBytes[port][qIndex])
			{
				grow = (m_usedEgressQSharedBytes[port][qIndex] - last_inputEgressBytes[port][qIndex]) * 8.0 / 1000.0 / interval;
			}
			if(m_id == 1 && qIndex == 3)
			{
				//std::cout << Simulator::Now().GetMicroSeconds()/1000.0 << " m_id: "<< m_id << " port: "<<port <<" qIndex: "<<qIndex << " "<< grow << std::endl;
			}
			//m_inputEgressBytes[port][qIndex] = 0;
			last_caltime[port][qIndex] = Simulator::Now().GetMicroSeconds();
			last_inputEgressBytes[port][qIndex] = m_usedEgressQSharedBytes[port][qIndex];
		}
		...
		BroadcomNode::UpdateEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize)
	{
		...
		if (m_usedEgressQMinBytes[port][qIndex] + psize < m_q_min_cell)	//guaranteed
		{
		    ...
		    m_usedEgressPGPackets[port][qIndex] += 1;
			return;
		}
		else
		{
		...
		BroadcomNode::RemoveFromEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize)
	{
		...
		else
		{
			...
			if(m_id == 0 && qIndex == 3 && port == 2)
			{
				//std::cout << Simulator::Now().GetMicroSeconds()/1000.0 << " m_id: "<< m_node->GetId() /*<< " inDev: "<<inDev */<<" m_ifIndex: "<<m_ifIndex << " "<< grow[qIndex] <<" Interval: "<< interval<< " last_caltime: "<< last_caltime[qIndex]<< std::endl;
				//std::cout << Simulator::Now().GetMicroSeconds()/1000.0 << " m_id: "<< m_id << " port: "<< port /*<<" qIndex: "<<qIndex */<<" " << m_usedEgressQSharedBytes[port][qIndex] << std::endl;
		...
		BroadcomNode::GetResumeClasses(uint32_t port, uint32_t qIndex)
	{
		...
		else
		{
			...
			{
				if(port == 1 && m_id == 1){
				//std::cout<< port <<" sending resume !! m_usedIngressPGBytes[port][qIndex] = " << m_usedIngressPGBytes[port][qIndex]/1030 <<" "<<Simulator::Now().GetMicroSeconds() <<std::endl;
				}
				return true;
			}
		}
	...
	uint32_t
		BroadcomNode::GetEgressPackets(uint32_t port, uint32_t qIndex)
	{
		return m_usedEgressPGPackets[port][qIndex];
	}
