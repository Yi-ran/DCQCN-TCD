...
public:
  ...
  static const uint32_t fCnt = 12182; // Max number of flows on a NIC, for TX and RX respectively. TX+RX=fCnt*2
...
protected:
  ...
  /// Wrapper as an schedulable function
  void StopVictim(unsigned qIndex);
  ...
  EventId m_victimEvt[qCnt];  //< Keeping the resume timer, used for root detection
  ...
  struct DataAccount{
    ...
    double prevtime;
    uint32_t sendnum;
    uint32_t ecnnum1; //quasi
    uint32_t ecnnum2; //quasi
    uint32_t ecnnum3; //real
  };
  ...
  double lastresume_time[qCnt];
  uint32_t m_victim[qCnt];
  uint32_t m_sendout[qCnt];
  bool m_npecn;
  bool m_rootecn;
  bool m_rootecncc;
  uint32_t m_interval; //us
  double m_rate_prevtime;//us
  double m_realrate_prevtime;//us

  std::vector<ECNAccount> *m_ecn_source;
  std::vector<DataAccount> *m_data_source;
  std::vector<ACKAccount> *m_ack_source;
 ...
 // void ProcessNACK(Ptr<Packet> packet);
  ...
  double last_caltime[qCnt];
  //uint32_t queue_upon_pause[qCnt];
  //uint32_t queue_upon_trigger_pause[qCnt];
  //bool queue_upon_trigger_pause_first[qCnt];
  //uint32_t queue_upon_resume[qCnt];
  //double queue_upon_trigger_pause_time[qCnt];
  //double queue_upon_pause_time[qCnt];
  //double grow[qCnt];
  double PAUSE_period[qCnt];

  uint32_t pause_num[qCnt];
  double pause_time[qCnt];

  uint32_t pausedPacketsNum[qCnt];
  uint32_t queue_upon_resume[qCnt];
  double last_timeout[qCnt];
  uint32_t last_timeout_queue[qCnt];
  uint8_t pre_state[qCnt];
