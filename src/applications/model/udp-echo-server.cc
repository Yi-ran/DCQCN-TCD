...
UdpEchoServer::GetTypeId (void)
{
  ...
  .AddAttribute ("Total",
                   ...
                   UintegerValue (0),
                   MakeUintegerAccessor (&UdpEchoServer::m_total),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("ClientStart",
                   "ClientStarttime.", TimeValue (Seconds (0.0)),
                   MakeTimeAccessor (&UdpEchoServer::m_client_start),
                   MakeTimeChecker())
    .AddTraceSource ("QpComplete", "A qp completes.",
                   MakeTraceSourceAccessor (&UdpEchoServer::m_traceQpComplete))
  ;
  return tid;
}
...
UdpEchoServer::StartApplication (void)
{
  ...
  m_qpCompleteCallback = MakeCallback(&UdpEchoServer::QpComplete, this);
...
void
UdpEchoServer::QpComplete (Ptr<Socket> qp)
{
