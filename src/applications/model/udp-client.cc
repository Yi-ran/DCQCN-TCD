...
UdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpClient")
    ...
    .AddAttribute ("PacketSize",
                   ...
                   MakeUintegerChecker<uint32_t> (14,40000))
  ;
  return tid;
}
...
UdpClient::UdpClient ()
{
  ...
  uv->SetStream(1);
  uv->SetAttribute ("Min", DoubleValue (0.45));
  uv->SetAttribute ("Max", DoubleValue (0.55));
}
...
UdpClient::Send (void)
{
  ...
  for (uint32_t i=0;i<dn;i++)
  {
	  ...
	  double tmp = (buffer*8.0-1500*8.0)/40/1000000000*0.95; //0.95 is for conservative. assuming 40Gbps link.
          //double tmp = (buffer*8.0-1500*8.0)/100/1000000000*0.95;
  ...
  if(node->GetId() == 61)
  {
    //next_avail = 0.000000815 * 0.83; //20Gbps
    //next_avail = 0.000000815 * 4;
  }
  if(node->GetId()== 20)
 {
   //next_avail = 0.000000815 * 0.83; //20Gbps
  ...
  if (m_sent < m_allowed)
    {
      m_sendEvent = Simulator::Schedule (Seconds(next_avail * uv->GetValue()/*UniformVariable(0.45,0.55).GetValue()*/), &UdpClient::Send, this);
    }
