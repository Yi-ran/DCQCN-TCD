...
UdpServer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpServer")
    ...
    .AddAttribute ("Total",
                   "Total data (packets) should receive.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&UdpServer::m_total),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("ClientStart",
                   "ClientStarttime.", TimeValue (Seconds (0.0)),
                   MakeTimeAccessor (&UdpServer::m_client_start),
                   MakeTimeChecker())
    .AddTraceSource ("QpComplete", "A qp completes.",
                   MakeTraceSourceAccessor (&UdpServer::m_traceQpComplete))
    ;
  return tid;
}
...
UdpServer::StartApplication (void)
{
  ...
  m_qpCompleteCallback = MakeCallback(&UdpServer::QpComplete, this);
  ...
  while ((packet = socket->RecvFrom (from)))
    {
	  ...
	  */
     //yiran: complete
      if(m_receivednum >= m_total && m_total)
      {
        QpComplete(socket);
        //std::cout << "flow compelte at " << Simulator::Now().GetMicroSeconds() << std::endl;
      }
    }
}
...
void
UdpServer::QpComplete (Ptr<Socket> qp)
{
  //m_traceQpComplete(qp,m_total,m_client_start);
  m_traceQpComplete(qp,m_received,m_client_start);
}
