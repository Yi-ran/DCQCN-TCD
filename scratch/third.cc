...
double pause_time = 5, simulator_stop_time = 2.01, app_start_time = 0.0, app_stop_time = 3.0;
...
bool enable_npecn = false;
bool enable_rootecn = false;
bool enable_rootecncc = false;

uint32_t rootecn_interval = 40;
uint32_t seednum = 6;
std::string fct_output_file = "fct.txt";
FILE *fct_output = NULL;

void qp_finish(/*FILE* fout, */uint32_t sid, Ptr<Socket> qp, uint32_t size, Time starttime){
	//fprintf(fout, "%lu QP complete\n", Simulator::Now().GetMicroSeconds());
	Time delta = Simulator::Now() - starttime;
	double fct = delta.ToDouble (Time::S);
	double start = starttime.ToDouble (Time::S);
	uint32_t did = qp->GetNode()->GetId();
        double basefct = 0;
        if( (sid / 25) == (did / 25))
	{
		//same pod
		uint32_t src = sid - ((sid)/ 25) * 25;
		uint32_t dst = did - ((did) / 25) * 25;
		if((src / 5) != (dst / 5))
		{
			basefct = size / 5000000000.0 + 0.000016;
		}
		else
		{
			basefct = size / 5000000000.0 + 0.000008;
		}
		
	}
	else
	{
		basefct = size / 5000000000.0 + 0.000024;
	}
	if(fct_output != NULL)
	{
		fprintf(fct_output, "%d %d %u %f %.8f %.8f\n", sid, qp->GetNode()->GetId(), size, start, fct, basefct);//size in bytes
		fflush(fct_output);
	}
	//std::cout<<Simulator::Now()<<" "<<sid<<" -> "<<qp->GetNode()->GetId()<<" "<<size*1000<<" "<<starttime<<" "<<fct<<std::endl;
}
...
//#endif
		while (!conf.eof())
		{
			...
			else if (key.compare("SEED_NUMBER") == 0)
			{
				uint32_t v;
				conf >> v;
				seednum = v;
				std::cout << "SEED_NUMBER\t\t" << seednum << "\n";
			}
			else if (key.compare("ENABLE_NPECN") == 0)
			{
				uint32_t v;
				conf >> v;
				enable_npecn = v;
				if (enable_npecn)
					std::cout << "ENABLE_NPECN\t\t\t" << "Yes" << "\n";
				else
					std::cout << "ENABLE_NPECN\t\t\t" << "No" << "\n";
			}
			else if (key.compare("ENABLE_ROOTECN") == 0)
			{
				uint32_t v;
				conf >> v;
				enable_rootecn = v;
				if (enable_rootecn)
					std::cout << "ENABLE_ROOTECN\t\t\t" << "Yes" << "\n";
				else
					std::cout << "ENABLE_ROOTECN\t\t\t" << "No" << "\n";
			}
			else if (key.compare("ENABLE_ROOTECNCC") == 0)
			{
				uint32_t v;
				conf >> v;
				enable_rootecncc = v;
				if (enable_rootecncc)
					std::cout << "ENABLE_ROOTECNCC" << "Yes" << "\n";
				else
					std::cout << "ENABLE_ROOTECNCC" << "No" << "\n";
			}
			else if (key.compare("ROOTECN_INTERVAL") == 0)
			{
				uint32_t v;
				conf >> v;
				rootecn_interval = v;
				std::cout << "ROOTECN_INTERVAL\t\t" << rootecn_interval << "\n";
			}
	...
	NS_ASSERT(enable_npecn + enable_rootecn < 2); //packet level ecmp and flow level ecmp are exclusive
	...
	Config::SetDefault("ns3::QbbNetDevice::EnableNPECN", BooleanValue(enable_npecn));
	Config::SetDefault("ns3::QbbNetDevice::EnableRootECN", BooleanValue(enable_rootecn));
	Config::SetDefault("ns3::QbbNetDevice::RootECNInterval", UintegerValue(rootecn_interval));
	Config::SetDefault("ns3::QbbNetDevice::EnableRootECNCC", BooleanValue(enable_rootecncc));
	...
	uint32_t node_num;
        uint32_t switch_num, link_num, flow_num, trace_num, tcp_flow_num;
	...
	for (uint32_t i = 0; i < flow_num; i++)
	{
		...
		uint32_t flowsize  = maxPacketCount;
		uint8_t onepacket = 0;
		maxPacketCount = (flowsize - 1) / 1000 + 1;	//bytes -> packets
		if(flowsize < 1000 && maxPacketCount == 1)
		{
			onepacket = 1;
		}
		...
		if (send_in_chunks)
		{
			...
			server0.SetAttribute("Total", UintegerValue(maxPacketCount));
			server0.SetAttribute("ClientStart", TimeValue(Time::FromDouble (start_time, Time::S)));
			...
			{
				client0.SetAttribute("PacketSize", UintegerValue(flowsize));
			}
			ApplicationContainer apps0c = client0.Install(n.Get(src));
			apps0c.Start(Seconds(start_time));
			apps0c.Stop(Seconds(stop_time));
			apps0s.Get(0)->TraceConnectWithoutContext("QpComplete", MakeBoundCallback (&qp_finish, n.Get(src)->GetId()));
